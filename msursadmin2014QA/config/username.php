...
if(isset($_GET["state"]))
{
	...
	if($state=='username')
	{
		if(isset($_GET["process"]))
		{
			...
			if($process=='update')
			{
				$username = mysql_real_escape_string(stripslashes(trim(@$_POST["user"])));
				$user_pass = mysql_real_escape_string(stripslashes(trim(@$_POST["user_password"])));
				$md_pass = md5($user_pass);
				echo "<div class='error_list'>\n";
				$emptyValue=0;
				if(empty($username)){ echo "<h2>თქვენ უნდა შეიყვანოთ <strong>მომხმარებელი</strong></h2>"; $emptyValue = 1; }
		...
		echo "<form action='?admin=configuration&amp;state=".$state."&amp;process=update' method='post' name='forma' class='insert_update_data'>
			<fieldset>
				<legend>მომხმარებლის რედაქტირება</legend>
				<label for='user'>მომხმარებელი</label>
				<input type='text' size='40' name='user' value='".$user."' id='user' />
