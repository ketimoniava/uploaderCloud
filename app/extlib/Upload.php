...
class Upload {
	...
	public function __construct($destination) {
		...
		//create finfo object
		// aq chirdeba es klasi, failis informaciis klasia ra uceb ro gitxra
		// danarcheni shen tviton shegidzlia naxo
	...
	protected function save_file() {		
		...
		//set full path
		$this->file['full_path'] = $this->destination . $this->filename .".". $this->extension;		
                // IF IN ANY UNPOSSIBLE WAY FILE EXISTS CREATE NEW NAME
                if($this->file_exist()) {
                    //return $this->save_file();
                    die("file exists: {$this->file[full_path]}");
                }                
	...
	protected function set_file_array($file) {
		...
		$this->tmp_name  = $file['tmp_name'];
                ...
                $this->extension = $this->get_file_extension();
                
                // MOVED HERE CREATING NEW NAME
                $this->create_new_filename();
	...
	protected function get_file_extension() {
		$d = explode('.',$this->file_post['name']);
		return strtolower(end($d));
	 ...
	 * @return bool
	 */
	protected function file_exist() {
	
		return is_file($this->file['full_path']);
	...
	protected function create_new_filename() {
		...
		$this->filename = FileHelper::generateRandomName('PH',13);
	}
